...
1. Выводи ТОЛЬКО распознанный текст. Никаких преамбул, послесловий, примечаний, сносок, скобочных комментариев («Текст с изображения:», «(Примечание: ...)», «(Note: ...)», «(Нб: ...)» и т.п.). Абсолютно ничего кроме самого текста.
...
9. Водяные знаки, логотипы, подписи каналов — игнорируй полностью, не упоминай их.

Если в запросе пользователя есть КОНТЕКСТ — это обязательные инструкции постобработки. Выполняй их СТРОГО: замены слов, сокращения повторов букв, любые правила написания — всё применяется к финальному тексту перед выводом. Не цитируй и не упоминай контекст в ответе."""
...
def _sanitize_output(text: str) -> str:
    ...
    # Убираем строки с мета-комментариями модели в скобках
    text = re.sub(r"^\s*\((?:Нб|Note|Примечание|Заметка|NB|PS|P\.S)[:\.].*\)\s*$", "", text, flags=re.MULTILINE | re.IGNORECASE)
